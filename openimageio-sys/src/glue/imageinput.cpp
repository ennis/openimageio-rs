...
void OIIO_ImageInput_delete(OIIO_ImageInput *in) { delete OIIO_RS_CAST(ImageInput, in); }
...
OIIO_ImageInput *OIIO_ImageInput_create(OIIO_StringRef filename, OIIO_StringRef plugin_searchpath) {
  std::string s_filename{filename.ptr, filename.len};
  OIIO::string_view sv_plugin_path{plugin_searchpath.ptr, plugin_searchpath.len};
  return (OIIO_ImageInput *)OIIO::ImageInput::create(s_filename, false, nullptr, sv_plugin_path)
      .release();
}

const char *OIIO_ImageInput_geterror(const OIIO_ImageInput *in) {
  std::string sstring = OIIO_RS_CAST_CONST(ImageInput, in)->geterror();
  return helpers::makeCString(sstring);
}

const char *OIIO_ImageInput_format_name(OIIO_ImageInput *in) {
  return OIIO_RS_CAST(ImageInput, in)->format_name();
}

bool OIIO_ImageInput_valid_file(OIIO_ImageInput *in, OIIO_StringRef filename) {
  std::string s_name{filename.ptr, filename.len};
  return OIIO_RS_CAST(ImageInput, in)->valid_file(s_name);
}
...
bool OIIO_ImageInput_open2(OIIO_ImageInput *in, OIIO_StringRef name, OIIO_ImageSpec *newspec) {
  ...
  return OIIO_RS_CAST(ImageInput, in)->open(s_name, *OIIO_RS_CAST(ImageSpec, newspec));
}

const OIIO_ImageSpec *OIIO_ImageInput_spec(const OIIO_ImageInput *in) {
  const OIIO::ImageSpec *spec = &(OIIO_RS_CAST_CONST(ImageInput, in)->spec());
...
bool OIIO_ImageInput_supports(const OIIO_ImageInput *in, OIIO_StringRef feature) {
  ...
  return OIIO_RS_CAST_CONST(ImageInput, in)->supports(s_feature);
}

bool OIIO_ImageInput_close(OIIO_ImageInput *in) { return OIIO_RS_CAST(ImageInput, in)->close(); }

int OIIO_ImageInput_current_subimage(const OIIO_ImageInput *in) {
  return OIIO_RS_CAST_CONST(ImageInput, in)->current_subimage();
}

bool OIIO_ImageInput_seek_subimage(OIIO_ImageInput *in, int subimage, OIIO_ImageSpec *newspec) {
  return OIIO_RS_CAST(ImageInput, in)->seek_subimage(subimage, *OIIO_RS_CAST(ImageSpec, newspec));
}

int OIIO_ImageInput_current_miplevel(const OIIO_ImageInput *in) {
  return OIIO_RS_CAST_CONST(ImageInput, in)->current_miplevel();
}
...
bool OIIO_ImageInput_seek_subimage_miplevel(OIIO_ImageInput *in, int subimage, int miplevel,
  ...
  return OIIO_RS_CAST(ImageInput, in)
      ->seek_subimage(subimage, miplevel, *OIIO_RS_CAST(ImageSpec, newspec));
}
...
bool OIIO_ImageInput_read_image_format2(OIIO_ImageInput *in, int chbegin, int chend,
  ...
  return OIIO_RS_CAST(ImageInput, in)
      ->read_image(chbegin, chend, helpers::unwrapTypeDesc(format), data, xstride, ystride,
                   zstride, cbk, cbk_data);
}

bool OIIO_ImageInput_read_scanline_format(OIIO_ImageInput *in, int y, int z, OIIO_TypeDesc format,
                                          void *data, stride_t xstride) {
  return OIIO_RS_CAST(ImageInput, in)
      ->read_scanline(y, z, helpers::unwrapTypeDesc(format), data, xstride);
}
