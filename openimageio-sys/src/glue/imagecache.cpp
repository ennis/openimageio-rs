...
void OIIO_ImageCache_destroy(OIIO_ImageCache *x, bool teardown) {
  OIIO::ImageCache::destroy(OIIO_RS_CAST(ImageCache, x), teardown);
}

const char *OIIO_ImageCache_geterror(const OIIO_ImageCache *x) {
  std::string str = OIIO_RS_CAST_CONST(ImageCache, x)->geterror();
  return helpers::makeCString(str);
}

const char *OIIO_ImageCache_getstats(const OIIO_ImageCache *x, int level) {
  std::string str = OIIO_RS_CAST_CONST(ImageCache, x)->getstats(level);
  return helpers::makeCString(str);
}

void OIIO_ImageCache_reset_stats(OIIO_ImageCache *x) { OIIO_RS_CAST(ImageCache, x)->reset_stats(); }

void OIIO_ImageCache_invalidate(OIIO_ImageCache *x, OIIO_StringRef filename) {
  OIIO::ustring s{filename.ptr, filename.len};
  OIIO_RS_CAST(ImageCache, x)->invalidate(s);
}

void OIIO_ImageCache_invalidate_all(OIIO_ImageCache *x, bool force) {
  OIIO_RS_CAST(ImageCache, x)->invalidate_all(force);
}

bool OIIO_ImageCache_attribute(OIIO_ImageCache *x, OIIO_StringRef name, OIIO_TypeDesc type,
                               const void *val) {
  return OIIO_RS_CAST(ImageCache, x)
      ->attribute(OIIO::string_view{name.ptr, name.len}, helpers::unwrapTypeDesc(type), val);
}

bool OIIO_ImageCache_getattribute(OIIO_ImageCache *x, OIIO_StringRef name, OIIO_TypeDesc type,
                                  void *val) {
  return OIIO_RS_CAST(ImageCache, x)
      ->getattribute(OIIO::string_view{name.ptr, name.len}, helpers::unwrapTypeDesc(type),
                     val);
}

OIIO_ImageCache_Perthread *
OIIO_ImageCache_get_perthread_info(OIIO_ImageCache *x, OIIO_ImageCache_Perthread *thread_info) {
  return (OIIO_ImageCache_Perthread *)OIIO_RS_CAST(ImageCache, x)
      ->get_perthread_info(OIIO_RS_CAST(ImageCache::Perthread, thread_info));
}

OIIO_ImageCache_Perthread *OIIO_ImageCache_create_perthread_info(OIIO_ImageCache *x) {
  return (OIIO_ImageCache_Perthread *)OIIO_RS_CAST(ImageCache, x)->create_thread_info();
}

void OIIO_ImageCache_destroy_perthread_info(OIIO_ImageCache *x,
                                            OIIO_ImageCache_Perthread *thread_info) {
  OIIO_RS_CAST(ImageCache, x)->destroy_thread_info(OIIO_RS_CAST(ImageCache::Perthread, thread_info));
}
...
OIIO_ImageCache_ImageHandle *OIIO_ImageCache_get_image_handle(OIIO_ImageCache *x,
  ...
  auto thread_info = OIIO_RS_CAST(ImageCache, x)->get_perthread_info();
  return (OIIO_ImageCache_ImageHandle *)OIIO_RS_CAST(ImageCache, x)
      ->get_image_handle(OIIO::ustring{name.ptr, name.len}, thread_info);
}

bool OIIO_ImageCache_good(OIIO_ImageCache *x, OIIO_ImageCache_ImageHandle *file) {
  return OIIO_RS_CAST(ImageCache, x)->good(OIIO_RS_CAST(ImageCache::ImageHandle, file));
}
...
bool OIIO_ImageCache_get_image_info(OIIO_ImageCache *x, OIIO_StringRef filename, int subimage,
                                    ...
                                    void *data) {
  return OIIO_RS_CAST(ImageCache, x)
      ->get_image_info(OIIO::ustring{filename.ptr, filename.len}, subimage, miplevel,
                       OIIO::ustring{dataname.ptr, dataname.len},
                       helpers::unwrapTypeDesc(datatype), data);
}
...
bool OIIO_ImageCache_get_image_info_by_handle(OIIO_ImageCache *x, OIIO_ImageCache_ImageHandle *file,
  ...
  return OIIO_RS_CAST(ImageCache, x)
      ->get_image_info(OIIO_RS_CAST(ImageCache::ImageHandle, file),
                       OIIO_RS_CAST(ImageCache::Perthread, thread_info), subimage, miplevel,
                       OIIO::ustring{dataname.ptr, dataname.len},
                       helpers::unwrapTypeDesc(datatype), data);
}
...
bool OIIO_ImageCache_get_imagespec(OIIO_ImageCache *x, OIIO_StringRef filename,
  ...
  return OIIO_RS_CAST(ImageCache, x)
      ->get_imagespec(OIIO::ustring{filename.ptr, filename.len}, *OIIO_RS_CAST(ImageSpec, spec),
                      subimage, miplevel, native);
}
...
bool OIIO_ImageCache_get_imagespec_by_handle(OIIO_ImageCache *x, OIIO_ImageCache_ImageHandle *file,
  ...
  // Temporary fix for OIIO bug if Perthread is NULL
  if (!thread_info) {
    thread_info = (OIIO_ImageCache_Perthread *)OIIO_RS_CAST(ImageCache, x)->get_perthread_info();
  }

  return OIIO_RS_CAST(ImageCache, x)
      ->get_imagespec(OIIO_RS_CAST(ImageCache::ImageHandle, file),
                      OIIO_RS_CAST(ImageCache::Perthread, thread_info), *OIIO_RS_CAST(ImageSpec, spec),
                      subimage, miplevel, native);
}

const char *OIIO_ImageCache_resolve_filename(OIIO_ImageCache *x, OIIO_StringRef filename) {
  auto str = OIIO_RS_CAST(ImageCache, x)->resolve_filename(std::string{filename.ptr, filename.len});
  return helpers::makeCString(str);
}
...
bool OIIO_ImageCache_get_pixels(OIIO_ImageCache *x, OIIO_StringRef filename, int subimage,
  ...
  return OIIO_RS_CAST(ImageCache, x)
      ->get_pixels(OIIO::ustring{filename.ptr, filename.len}, subimage, miplevel, xbegin, xend,
                   ybegin, yend, zbegin, zend, helpers::unwrapTypeDesc(format), result);
}

bool OIIO_ImageCache_get_pixels_by_handle(OIIO_ImageCache *x, OIIO_ImageCache_ImageHandle *file,
                                          OIIO_ImageCache_Perthread *thread_info, int subimage,
                                          int miplevel, int xbegin, int xend, int ybegin, int yend,
                                          int zbegin, int zend, OIIO_TypeDesc format,
                                          void *result) {

  return OIIO_RS_CAST(ImageCache, x)
      ->get_pixels(OIIO_RS_CAST(ImageCache::ImageHandle, file),
                   OIIO_RS_CAST(ImageCache::Perthread, thread_info), subimage, miplevel, xbegin, xend,
                   ybegin, yend, zbegin, zend, helpers::unwrapTypeDesc(format), result);
}
...
bool OIIO_ImageCache_get_pixels_stride(OIIO_ImageCache *x, OIIO_StringRef filename, int subimage,
  ...
  return OIIO_RS_CAST(ImageCache, x)
      ->get_pixels(OIIO::ustring{filename.ptr, filename.len}, subimage, miplevel, xbegin, xend,
                   ...
                   helpers::unwrapTypeDesc(format), result, xstride, ystride, zstride,
